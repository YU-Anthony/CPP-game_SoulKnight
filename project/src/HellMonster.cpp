...
HellMonster::HellMonster(Scyyz4Engine* pEngine, Stage* stage, int indexInArray, int xSize, int ySize, int worldXPos, int worldYPos, Scyyz4Object* player, std::string imageName)
...
void HellMonster::PerformMovement()
{
	/*detectCollision();*/
	...
	randomMove(mx_position,my_position,dir);
...
void HellMonster::drawHP()
{

	hOutlineX = mx_position+5;
...
void HellMonster::detectCollision()
{
	insideBoundary();
	...
	HellMonster* monster;
	for (int iObjectId = 6; (monster = (HellMonster*)m_pMainEngine->getDisplayableObject(iObjectId)) != NULL; iObjectId++)
	{
		/*printf("onjectID:%d\n", iObjectId);*/
...
void HellMonster::insideBoundary()
{
